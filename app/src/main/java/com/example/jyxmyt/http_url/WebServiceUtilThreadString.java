 ...
 *
 */
public class WebServiceUtilThreadString extends Thread{
	...
	public String getAllProvince(String method, List<NameValuePair> list) {
		try {
	...
	public void run() {
		String  inputStream=getAllProvince(method, list);
		if (handler==null) {
			return;
		}
	 ...
	 * 
	 * @param handler 
	 * @param method 访问服务的url
	 * @param list 服务需要的字段  集合
	 * @param what 返回到activit的handler中的what值
	 */
	public WebServiceUtilThreadString(Handler handler, String method,
	 ...
	 */
	public WebServiceUtilThreadString(Handler handler, String method,
			List<NameValuePair> list, int what, int page) {
		super();
		this.handler = handler;
		this.method = method;
		this.list = list;
